...
describe('mutateStateAddNode', () => {
  ...
  it('add sibling of level 2 with undefined parentId, between two existing nodes', () => {
    ...
    expect(state.nodesDict[10025]).toBeDefined();
    expect(state.nodesDict[10025].parent).toEqual(1002);
    expect(state.viewsDict["Main"].visibleNodesList).toContain(10025);

    // new node should be between 10022 and 10023
    const pos10025 = state.viewsDict["Main"].visibleNodesList.indexOf(10025);
    const pos10022 = pos10025 - 1;
    const pos10023 = pos10025 + 1;
    expect(state.viewsDict["Main"].visibleNodesList[pos10022]).toEqual(10022);
    expect(state.viewsDict["Main"].visibleNodesList[pos10023]).toEqual(10023);
